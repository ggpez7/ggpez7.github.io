...
ZH_EN_PHRASE_MAP = [
    ("江淮轻卡", "Jianghuai light truck"),
    ("江淮", "Jianghuai"),
    ("吉利商用车量产", "Geely commercial vehicle mass-production program"),
    ("吉利", "Geely"),
    ("商用车", "commercial vehicle"),
    ("量产", "mass-production"),
    ("徐州新的一轮", "new Xuzhou round"),
    ("徐州", "Xuzhou"),
    ("新的一轮", "new round"),
    ("铁路沿线安全光纤综合监测系统", "railway right-of-way fiber-optic monitoring system"),
    ("围岩变形智能监测系统", "intelligent surrounding-rock deformation monitoring system"),
    ("技防监控项目", "security monitoring project"),
    ("隧道口增设智能视频", "tunnel-portal intelligent video monitoring upgrade"),
    ("隧道口", "tunnel portal"),
    ("普速铁路", "conventional railway"),
    ("普铁", "conventional railway"),
    ("高铁", "high-speed railway"),
    ("城际铁路", "intercity railway"),
    ("兰张三四线铁路", "Lanzhou-Zhangye third and fourth line railway"),
    ("东大山隧道", "Dongdashan Tunnel"),
    ("中铁十局", "China Railway No. 10 Engineering Group"),
    ("上海局集团公司", "Shanghai Railway Group"),
    ("上海局", "Shanghai Railway Bureau"),
    ("昆明局集团公司", "Kunming Railway Group"),
    ("贵广", "Guiyang-Guangzhou"),
    ("广西段", "Guangxi section"),
]


def translate_chinese_project_name(text: str) -> str:
    translated = norm_space(text)
    for zh, en in sorted(ZH_EN_PHRASE_MAP, key=lambda item: len(item[0]), reverse=True):
        translated = translated.replace(zh, f" {en} ")
    translated = translated.replace("项目", " project ")
    translated = translated.replace("合同", " contract ")
    translated = translated.replace("监测系统", " monitoring system ")
    translated = translated.replace("铁路", " Railway ")
    translated = re.sub(r"([A-Za-z])\s+([A-Z][a-z])", r"\1 \2", translated)
...
def generic_translate_chinese_sentence(text: str, company_name: str) -> str:
    ...
    replacements = [
        ("台湾第一批", "the first Taiwan batch"),
        ("已经发货", "has been shipped"),
        ("预计", "is expected to"),
        ("结算", "settle"),
        ("第一批销售收入", "the first batch of sales revenue"),
        ("已完成核名并补充注册资料", "completed name registration and supplemental filing documents"),
        ("待完成预计2月底前完成", "with completion expected before the end of February"),
        ("待完成", "pending completion"),
        ("小规模测试", "small-scale testing"),
        ("完成清关入库", "complete customs clearance and warehousing"),
        ("上架销售", "launch for sale"),
        ("业务分润情况不及预期", "business revenue-sharing performance was below expectations"),
        ("待进一步观察评估", "and will continue to be monitored and assessed"),
        ("城市合伙人业务进展", "city-partner business progress"),
        ("下一轮融资计划", "next fundraising plan"),
        ("资金需求", "funding needs"),
        ("完成首批非wurth品牌产品采购上架", "complete procurement and listing of the first batch of non-Wurth branded products"),
        ("实现收入", "generate revenue"),
        ("货物寻找现在的进展如何", "sourcing progress"),
        ("台湾公司设立的最新进展是什么", "Taiwan entity formation progress"),
        ("大陆业务的进展情况", "Mainland China business progress"),
        ("四季度的主要里程碑和时间线是什么", "Q4 milestones and timeline"),
    ]
    for zh, en in replacements:
        translated = translated.replace(zh, en)
    translated = translated.replace("台湾", "Taiwan")
    translated = translated.replace("大陆", "Mainland China")
    translated = translated.replace("融资", "financing")
    translated = translated.replace("业务", "business")
    translated = translated.replace("进展", "progress")
    translated = translated.replace("货值", "order value")
    translated = translated.replace("货物", "goods")
    translated = translated.replace("发货", "shipment")
    translated = translated.replace("销售收入", "sales revenue")
    translated = translated.replace("完成", "complete")
    translated = translated.replace("清关", "customs clearance")
    translated = translated.replace("入库", "warehousing")
    translated = translated.replace("上架", "listing")
    translated = translated.replace("评估", "evaluate")
    translated = translated.replace("观察", "monitor")
    translated = translated.replace("预期", "expectations")
    translated = translated.replace("一季度", "Q1")
    translated = translated.replace("二季度", "Q2")
    translated = translated.replace("三季度", "Q3")
    translated = translated.replace("四季度", "Q4")
    translated = translated.replace("一月底前", "before the end of January")
    translated = translated.replace("2月底前", "before the end of February")
    translated = translated.replace("3月31日", "March 31")
    translated = translated.replace("如2026年要完成1千万的销售目标", "if the company targets RMB 10 million of sales in 2026")
    translated = translated.replace("待2026年一季度底按销售情况评估2026年资金需求", "The company will assess 2026 funding needs by the end of Q1 2026 based on sales performance")
    translated = translated.replace("以完成台湾市场备货", "to support inventory build-up for the Taiwan market")
    translated = translated.replace("Mycar", english_name)
    translated = re.sub(r"\s+", " ", translated)
    translated = translated.replace(" .", ".")
    translated = translated.replace(" ,", ",")
    translated = translated.strip(" .")
    if not re.search(r"[A-Za-z]", translated):
        return f"{english_name} update: {text}."
...
def polish_english_business_bullet(text: str, company_name: str) -> str:
    ...
    fixes = [
        ("batchmeister", "batch of Meister"),
        ("millionorder value", "million order value"),
        ("is expected toMarch 31settle", "is expected to settle by March 31"),
        ("pending completionis expected tobefore", "with completion expected before"),
        ("Wurthcity-partner", "Wurth city-partner"),
        ("salecompletebusinesssmall-scale testing", "sale for small-scale business testing"),
        ("第Q1", "Q1"),
    ]
    ...
    text = text.replace("My Car", "Mycar")
    text = text.replace("  ", " ")
    text = text.replace(" ,", ",")
    text = text.replace(" .", ".")
    text = text.strip()
...
def translate_chinese_update_bullet_to_english(text: str, company_name: str, translation_memory: dict[str, str] | None = None) -> str:
    ...
    text = convert_chinese_amounts_to_rmb_millions(text)

    quarter_match = re.search(r"(20\d{2})年\s*Q([1-4])", text, re.I)
    if quarter_match and "业务回顾" in text:
        return f"{quarter_match.group(1)} Q{quarter_match.group(2)} business review."

    if "应收账款" in text:
        amount_match = re.search(r"RMB ([\d.]+) million", text)
        amount = f" approximately RMB {amount_match.group(1)} million" if amount_match else ""
        trend = "with collections remaining stable" if any(token in text for token in ["平稳", "稳定"]) else "with collections increasing versus the previous quarter" if any(token in text for token in ["上涨", "增加"]) else ""
        tail = f", {trend}" if trend else ""
        return f"Accounts receivable collections were{amount}{tail}.".replace(" ,", ",")

    if "融资" in text:
        return f"{english_name} is evaluating the next financing round and the broader fundraising plan."

    if all(token in original_text for token in ["人员变动", "江淮", "AMT项目"]):
        annual_sales_match = re.search(r"年销售量约\s*(\d+(?:\.\d+)?)\s*万台", original_text)
        annual_sales = None
        if annual_sales_match:
            annual_sales = int(Decimal(annual_sales_match.group(1)) * Decimal("10000"))
        feb_match = re.search(r"(\d{2})年\s*2月", original_text)
        mar_match = re.search(r"(\d{2})年\s*3月", original_text)
        feb_year = f"20{feb_match.group(1)}" if feb_match else None
        mar_year = f"20{mar_match.group(1)}" if mar_match else None
        sales_text = f", with annual sales of approximately {annual_sales:,} units" if annual_sales else ""
        feb_text = f" in February {feb_year}" if feb_year else ""
        mar_text = f" in March {mar_year}" if mar_year else ""
        return (
            f"Due to significant personnel changes in Q3, {english_name} is currently focused on a single Jianghuai light-truck AMT project{sales_text}. "
            f"The first prototype vehicle has completed unloaded debugging in Changzhou and passed Jianghuai's initial expert acceptance. "
            f"It is expected to be shipped to Hefei for full-load and mixed-road-condition testing{feb_text}, with acceptance targeted{mar_text} before entering the small-batch validation phase."
        )

    if "批产" in text and "年收入达到" in text:
        amount_match = re.search(r"RMB ([\d.]+) million", text)
        amount = amount_match.group(1) if amount_match else None
        prefix = text.split("批产", 1)[0].replace("完成", "").rstrip("，,。 ")
        translated_name = translate_chinese_project_name(prefix)
        if amount:
            return f"{english_name} aims to complete mass production of the {translated_name} and achieve annual revenue of RMB {amount} million."
        return f"{english_name} aims to complete mass production of the {translated_name}."

    if "现金已经无法支持运营" in text:
        return f"{english_name}'s current cash position is no longer sufficient to support operations."

    if "上市" in text or "股权" in text:
        return f"{english_name} continued organizing historical, financial, related-party, business, and organizational materials to assess the most suitable listing venue."

    if all(token in text for token in ["整理历史资料", "整理财务资料", "梳理关联方", "梳理业务与组织结构"]):
        return f"{english_name} continued organizing historical records, financial materials, related-party information, and the business and organizational structure."

    if "持续开发交付" in text and "款项已经支付" in text:
        prefix = text.split("持续开发交付", 1)[0]
        translated_name = translate_chinese_project_name(prefix)
        translated_name = re.sub(r"\b(program|project)\s+project\b", r"\1", translated_name, flags=re.I)
        return f"The {translated_name} remained under development and delivery, and the second-phase payment was received."

    if "正在交割" in text:
        prefix = text.split("正在交割", 1)[0].rstrip("，,。 ")
        translated_name = translate_chinese_project_name(prefix)
        return f"The {translated_name} is in closing."

    if "第一批" in text and "发货" in text and "销售收入" in text:
        amount_match = re.search(r"RMB ([\d.]+) million", text)
        amount = f" with an order value of RMB {amount_match.group(1)} million" if amount_match else ""
        if "meister" in text.lower():
            return f"The first batch of Meister tools in Taiwan{amount} has been shipped, and the first batch of sales revenue is expected to be settled by March 31."
        return f"The first batch in Taiwan{amount} has been shipped, and the first batch of sales revenue is expected to be settled by March 31."

    if "核名" in text and "注册资料" in text:
        return "The Taiwan entity has completed name approval and supplemental registration documents, with completion expected before the end of February."

    if "清关入库" in text and "小规模测试" in text:
        return "The first batch of products is expected to complete customs clearance and warehousing in mid-January and be listed for sale by the end of January for small-scale business testing."

    if "业务分润情况不及预期" in text:
        return f"{english_name}'s revenue-sharing performance was below expectations, and the company will continue to monitor and evaluate the progress of the Wurth city-partner business."

    if "非wurth" in text.lower() and "采购上架" in text:
        return f"{english_name} plans to complete procurement and listing of the first batch of non-Wurth branded products to begin generating revenue in Q1 2026."

    if "计划2026年产出" in text and "台" in text:
        amount_match = re.search(r"计划2026年产出\s*([\d-]+)\s*台", text)
        units = amount_match.group(1) if amount_match else "planned"
        if "环卫车" in text and "物流车" in text:
            return f"Leveraging Xuzhou resources, the sanitation and logistics vehicle line targets production of {units} units in 2026."

    if "继续找主机厂做更多的量产项目" in text:
        if "NOA" in text:
            return f"For the highway NOA commercial vehicle line, {english_name} continued pursuing additional mass-production programs with OEMs."
        return f"{english_name} continued pursuing additional mass-production programs with OEMs."

    if any(token in text for token in ["项目", "合同", "签约", "增设", "监测系统", "技防监控", "铁路"]):
        amount_match = re.search(r"RMB ([\d.]+) million", text)
        amount = amount_match.group(1) if amount_match else None
        descriptor = re.sub(r"RMB [\d.]+ million", "", text)
        descriptor = descriptor.replace("签约", "").replace("合同", "").replace("累计收了", "").strip("，,。 ")
        translated_name = translate_chinese_project_name(descriptor)
        if amount:
            return f"{translated_name} with a contract value of RMB {amount} million."
